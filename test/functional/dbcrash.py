...
class ChainstateWriteCrashTest(Bitcoin5000TestFramework):
    ...
    def restart_node(self, node_index, expected_tip):
        ...
        while time.time() - time_start < 120:
            try:
                ...
                # If bitcoin5000d exits, then try again.  wait_for_node_exit()
                # should raise an exception if bitcoin5000d doesn't exit.
                self.wait_for_node_exit(node_index, timeout=10)
            self.crashed_on_restart += 1
            time.sleep(1)

        # If we got here, bitcoin5000d isn't coming back up on restart.  Could be a
        # bug in bitcoin5000d, or we've gotten unlucky with our dbcrash ratio --

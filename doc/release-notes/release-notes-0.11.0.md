Bitcoin5000 Core version 0.11.0 is now available from:

  <https://bitcoin5000.org/bin/bitcoin5000-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer BVK/kB fee to be
...
For example, add the following to `bitcoin5000.conf`:
...
related to the blockchain in the bitcoin5000 system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Bitcoin5000 Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Bitcoin5000 Core will 
...
For an example script see [bitcoin5000-submittx](https://github.com/laanwj/bitcoin5000-submittx).
...
- #5334 `cf87536` libbitcoin5000consensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new bitcoin5000-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only Bitcoin5000AmountField
- #5753 `0f44672` Add bitcoin5000 logo to about screen
...
- #5809 `46bfbe7` Add bitcoin5000-cli man page
...
- #5849 `41259ca` contrib/init/bitcoin5000d.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Bitcoin5000-Qt.app to Bitcoin5000-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for bitcoin5000-tx)

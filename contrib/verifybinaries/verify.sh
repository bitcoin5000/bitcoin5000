...
###   bitcoin5000core.org and bitcoin5000.org and compares them.
...
WORKINGDIR="/tmp/bitcoin5000_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoin5000core.org"
HOST2="https://bitcoin5000.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoin5000-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoin5000-',
...
if [ $? -ne 0 ]; then
   echo "bitcoin5000.org failed to provide signature file, but bitcoin5000core.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoin5000.org and bitcoin5000core.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bitcoin5000 Core binary release signing key installed?"
   fi

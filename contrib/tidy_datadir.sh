...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Bitcoin5000 database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Bitcoin5000 datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Bitcoin5000 datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Bitcoin5000 0.7 datadir."
    ;;
  3)
    echo "Detected Bitcoin5000 pre-0.8 datadir."
    ;;
  4)
    echo "Detected Bitcoin5000 0.8 datadir."
    ;;
esac

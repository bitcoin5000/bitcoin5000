...
The RPM spec file provided here is for Bitcoin5000-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitcoin5000.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bitcoin5000 git repository](https://github.com/bitcoin5000/bitcoin5000) but are not part of
...
Currently only `Source10` is used, for the example `bitcoin5000.conf` file.
...
in the bitcoin5000 git repository. At present only an SVG version of the Bitcoin5000
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitcoin5000
...
needed when 0.12.1 is released, a proper fix is already in the Bitcoin5000 git
master branch.
...
idea to build Bitcoin5000 Core against a static version of that library compiled
...
The `bitcoin5000.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitcoin5000-server` package. At this point, most RPM based
...
The `bitcoin5000-server` package should have SELinux support. How to properly do
...
Bitcoin5000-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitcoin5000/). He has been
packaging Bitcoin5000 for Fedora at least since 2012.
...
4. I renamed the `bitcoin5000` package that contains the Qt GUI to `bitcoin5000-core` as
that appears to be how the general population refers to it, in contrast to
`bitcoin5000-xt` or `bitcoin5000-classic`. I wanted to make sure the general population

...
ThresholdState AbstractThresholdConditionChecker::GetStateFor(
  ...
  while (cache.count(pindexPrev) == 0) {
    ...
    if (pindexPrev->GetMedianTimePast() < nTimeStart) {
      // Optimization: don't recompute down further, as we know every earlier
      // block will be before the start time
      cache[pindexPrev] = THRESHOLD_DEFINED;
      break;
    }
...
// return the numerical statistics of blocks signalling the specified BIP9
// condition in this current period
...
int AbstractThresholdConditionChecker::GetStateSinceHeightFor(
  ...
  // it is computed based on a pindexPrev whose height equals a multiple of
  // nPeriod - 1.
  // To ease understanding of the following height calculation, it helps to
  // remember that
  // right now pindexPrev points to the block prior to the block that we are
  // computing for, thus:
  // if we are computing for the last block of a period, then pindexPrev points
  // to the second to last block of the period, and
  // if we are computing for the first block of a period, then pindexPrev points
  // to the last block of the previous period.

...
bool IsWitnessStandard(const CTransaction &tx,
  ...
  for (unsigned int i = 0; i < tx.vin.size(); i++) {
    // We don't care if witness for this input is empty, since it must not be
    // bloated.
    // If the script is invalid without witness, it would be caught sooner or
    ...
    if (prevScript.IsPayToScriptHash()) {
      std::vector<std::vector<unsigned char>> stack;
      // If the scriptPubKey is P2SH, we try to extract the redeemScript
      // casually by converting the scriptSig
      // into a stack. We do not check IsPushOnly nor compare the hash as these
      // will be done later anyway.
      // If the check fails at this stage, we know that this txid must be a bad
      // one.
      if (!EvalScript(stack, tx.vin[i].scriptSig, SCRIPT_VERIFY_NONE,
                      BaseSignatureChecker(), SIGVERSION_BASE))
        return false;

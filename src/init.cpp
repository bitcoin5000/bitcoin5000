 ...
 * shared
 * between bitcoin5000d, and bitcoin5000-qt and non-server tools.
*/
...
void Shutdown() {
  ...
  RenameThread("bitcoin5000-shutoff");
...
std::string HelpMessage(HelpMessageMode mode) {
  ...
  strUsage += HelpMessageOpt(
      "-conf=<file>", strprintf(_("Specify configuration file (default: %s)"),
                                BITCOIN5000_CONF_FILENAME));
  if (mode == HMM_BITCOIN5000D) {
#if HAVE_DECL_DAEMON
    strUsage += HelpMessageOpt(
  ...
  strUsage += HelpMessageOpt(
      "-pid=<file>",
      strprintf(_("Specify pid file (default: %s)"), BITCOIN5000_PID_FILENAME));
#endif
  strUsage += HelpMessageOpt(
      "-prune=<n>",
...
void ThreadImport(std::vector<fs::path> vImportFiles) {
  ...
  RenameThread("bitcoin5000-loadblk");
...
/** Sanity checks
 *  Ensure that Bitcoin5000 is running in a usable environment with all
...
void InitLogging() {
  ...
  LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
  LogPrintf("Bitcoin5000 version %s\n", FormatFullVersion());
}
...
bool AppInitParameterInteraction() {
  ...
  if (gArgs.GetBoolArg("-disablefork", false)) {
    printf("NOTE: disabling fork; this node will run on the bitcoin5000 "
           "network\n");
    FORK_ALLOWED = false;
...
static bool LockDataDirectory(bool probeOnly) {
  ...
  // Make sure only a single Bitcoin5000 process is using the data directory.
...
// application initialization
  ...
  LogPrintf(
      "Using config file %s\n",
      GetConfigFile(gArgs.GetArg("-conf", BITCOIN5000_CONF_FILENAME)).string());
...
// ********************************************************* Step 8: load wallet
  ...
  GenerateBitcoin5000s(gArgs.GetBoolArg("-gen", DEFAULT_GENERATE),

...
unsigned int GetNextWorkRequired(const CBlockIndex *pindexLast,
  ...
  if (pindexLast->nHeight >= FORK_BLOCK &&
      pindexLast->nHeight <=
          FORK_BLOCK + params.DifficultyAdjustmentInterval()) {
    // We also need to use the post fork pow limit, since we haven't technically
    // forked yet
    if (!params.postForkPowLimit.IsNull() &&
        params.postForkPowLimit != params.powLimit) {
      nProofOfWorkLimit = UintToArith256(params.postForkPowLimit).GetCompact();
    }
...
unsigned int CalculateNextWorkRequired(const CBlockIndex *pindexLast,
  ...
  if ((bnNew * nActualTimespan) / (nActualTimespan - 1) < bnNew) {
    // Overflowing due to really high POW limit; do the opposite order
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits,
                      const Consensus::Params &params, uint256 *best) {
  bool fNegative;
  bool fOverflow;
  ...
  if (fNegative || bnTarget == 0 || fOverflow ||
      bnTarget > UintToArith256(params.postForkPowLimit.IsNull()
                                    ? params.powLimit
                                    : params.postForkPowLimit)) {
    return false;
  }
  ...
  if (UintToArith256(hash) > bnTarget) {
    if (best &&
        (*best == uint256() || UintToArith256(*best) > UintToArith256(hash))) {
      *best = hash;
      printf("- new best:\n\t%s\nvs\t%s\n", hash.ToString().c_str(),

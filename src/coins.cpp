...
void AddCoins(CCoinsViewCache &cache, const CTransaction &tx, int nHeight,
  ...
  for (size_t i = 0; i < tx.vout.size(); ++i) {
    ...
    // Always set the possible_overwrite flag to AddCoin for coinbase txn, in
    // order to correctly
...
bool CCoinsViewCache::BatchWrite(CCoinsMap &mapCoins,
  ...
  for (CCoinsMap::iterator it = mapCoins.begin(); it != mapCoins.end();) {
    if (it->second.flags &
        CCoinsCacheEntry::DIRTY) { // Ignore non-dirty entries (optimization).
      ...
      if (itUs == cacheCoins.end()) {
        ...
        if ((it->second.flags & CCoinsCacheEntry::FRESH) &&
          ...
          throw std::logic_error("FRESH flag misapplied to cache entry for "
                                 "base transaction with spendable outputs");

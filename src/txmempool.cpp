...
// for each such descendant, also update the ancestor state to include the
// parent.
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
      ...
      if (piter != mapTx.end()) {
        ...
        if (parentHashes.size() + 1 > limitAncestorCount) {
          errString = strprintf("too many unconfirmed parents [limit: %u]",
                                limitAncestorCount);
          return false;
        }
      }
...
void CTxMemPool::UpdateForRemoveFromMempool(const setEntries &entriesToRemove,
  ...
  // For each entry, walk back all ancestors and decrement size associated with
  // this
  // transaction
  ...
  // each
  // transaction being removed and any mempool children (ie, update
  // setMemPoolParents
...
bool CTxMemPool::addUnchecked(const uint256 &hash, const CTxMemPoolEntry &entry,
  ...
  // In that case, our disconnect block logic will call
...
// adds to
// setDescendants. Assumes entryit is already a tx in the mempool and
// setMemPoolChildren
// is correct for tx and all descendants.
// Also assumes that if an entry is in setDescendants already, then all
// in-mempool descendants of it are already in setDescendants as well, so that
// we
...
void CTxMemPool::removeForReorg(const CCoinsViewCache *pcoins,
  ...
  for (indexed_transaction_set::const_iterator it = mapTx.begin();
    ...
    if (!CheckFinalTx(tx, flags) ||
      ...
      // So it's critical that we remove the tx and not depend on the
      // LockPoints.
...
void CTxMemPool::removeForBlock(const std::vector<CTransactionRef> &vtx,
  ...
  // Before the txs in the new block have been removed from the mempool, update
  // policy estimates
  if (minerPolicyEstimator) {
...
void CTxMemPool::check(const CCoinsViewCache *pcoins) const {
  ...
  for (indexed_transaction_set::const_iterator it = mapTx.begin();
    ...
    for (const CTxIn &txin : tx.vin) {
      // Check that every mempool transaction's inputs refer to available coins,
      // or other mempool tx's.
      indexed_transaction_set::const_iterator it2 =
          mapTx.find(txin.prevout.hash);
      if (it2 != mapTx.end()) {
  ...
  while (!waitingOnDependants.empty()) {
    ...
    if (!mempoolDuplicate.HaveInputs(entry->GetTx())) {
      ...
      bool fCheckResult =
          ...
          Consensus::CheckTxInputs(entry->GetTx(), state, mempoolDuplicate,
                                   nSpendHeight);
      assert(fCheckResult);
...
bool CCoinsViewMemPool::GetCoin(const COutPoint &outpoint, Coin &coin) const {
  ...
  // guaranteed to never
  // conflict with the underlying cache, and it cannot have pruned entries (as
  // it contains full)
  // transactions. First checking the underlying cache risks returning a pruned
  // entry instead.
...
void CTxMemPool::TrimToSize(size_t sizelimit,
  ...
  while (!mapTx.empty() && DynamicMemoryUsage() > sizelimit) {
    ...
    // We set the new mempool min fee to the feerate of the removed set, plus
    // the
    // "minimum reasonable fee rate" (ie some value under which we consider txn
    // to have 0 fee). This way, we don't allow txn to enter mempool with
    // feerate

...
CBlockHeaderAndShortTxIDs::CBlockHeaderAndShortTxIDs(const CBlock &block,
  ...
  for (size_t i = 1; i < block.vtx.size(); i++) {
    ...
    shorttxids[i - 1] =
        GetShortID(fUseWTXID ? tx.GetWitnessHash() : tx.GetHash());
  }
}
...
ReadStatus PartiallyDownloadedBlock::InitData(
  ...
  for (size_t i = 0; i < cmpctblock.prefilledtxn.size(); i++) {
    ...
    if ((uint32_t)lastprefilledindex > cmpctblock.shorttxids.size() + i) {
      // If we are inserting a tx at an index greater than our full list of
      // shorttxids
      // plus the number of prefilled txn we've inserted, then we have txn for
      // which we
  ...
  // (or don't)
  // Because well-formed cmpctblock messages will have a (relatively) uniform
  // distribution
  // of short IDs, any highly-uneven distribution of elements can be safely
  // treated as a
  // READ_STATUS_FAILED.
  ...
  for (size_t i = 0; i < cmpctblock.shorttxids.size(); i++) {
    ...
    // 1 / the number of buckets), that in the worst case the number of buckets
    // is
    // equal to S (due to std::unordered_map having a default load factor of
    // 1.0),
    // and that the chance for any bucket to exceed N elements is at most
    // buckets * (the chance that any given bucket is above N elements).
    // Thus: P(max_elements_per_bucket > N) <= S * (1 - cdf(binomial(n=S,p=1/S),
    // N)).
    // If we assume blocks of up to 16000, allowing 12 elements per bucket
    // should
    // only fail once per ~1 million block transfers (per peer and connection).
    if (shorttxids.bucket_size(shorttxids.bucket(cmpctblock.shorttxids[i])) >
        12)
    ...
    for (size_t i = 0; i < vTxHashes.size(); i++) {
      ...
      if (idit != shorttxids.end()) {
        if (!have_txn[idit->second]) {
          ...
          // If we find two mempool txn that match the short id, just request
          // it.
      ...
      // case,
      // the performance win of an early exit here is too good to pass up and
      // worth
      // the extra risk.
  ...
  for (size_t i = 0; i < extra_txn.size(); i++) {
    ...
    if (idit != shorttxids.end()) {
      if (!have_txn[idit->second]) {
        ...
        if (txn_available[idit->second] &&
            txn_available[idit->second]->GetWitnessHash() !=
                extra_txn[i].second->GetWitnessHash()) {
          txn_available[idit->second].reset();
          mempool_count--;
          extra_count--;
        }
  ...
  LogPrint(BCLog::CMPCTBLOCK, "Initialized PartiallyDownloadedBlock for block "
                              "%s using a cmpctblock of size %lu\n",

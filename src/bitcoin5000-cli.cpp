...
std::string HelpMessageCli() {
  ...
  strUsage += HelpMessageOpt(
      ...
      _("Send RPC for non-default wallet on RPC server (argument is wallet "
        "filename in bitcoin5000d directory, required if bitcoin5000d/-Qt runs "
...
static int AppInitRPC(int argc, char *argv[]) {
  ...
  if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||
    ...
    if (!gArgs.IsArgSet("-version")) {
      strUsage +=
          "\n" + _("Usage:") + "\n" +
          "  bitcoin5000-cli [options] <command> [params]  " +
          strprintf(_("Send command to %s"), _(PACKAGE_NAME)) + "\n" +
          "  bitcoin5000-cli [options] -named <command> [name=value] ... " +
          strprintf(_("Send command to %s (with named arguments)"),
                    _(PACKAGE_NAME)) +
          "\n" + "  bitcoin5000-cli [options] help                " +
          _("List commands") + "\n" +
          "  bitcoin5000-cli [options] help <command>      " +
    ...
    if (!GetAuthCookie(&strRPCUserColonPass)) {
      throw std::runtime_error(strprintf(
          _("Could not locate RPC credentials. No authentication cookie could "
            ...
            "(%s)"),
          GetConfigFile(gArgs.GetArg("-conf", BITCOIN5000_CONF_FILENAME))
              .string()
              .c_str()));
    }
  } else {

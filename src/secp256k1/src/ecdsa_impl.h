...
static const secp256k1_fe secp256k1_ecdsa_const_p_minus_order =
    SECP256K1_FE_CONST(0, 0, 0, 1, 0x45512319UL, 0x50B75FC4UL, 0x402DA172UL,
                       0x2FC9BAEEUL);

static int secp256k1_der_read_len(const unsigned char **sigp,
...
static int secp256k1_ecdsa_sig_parse(secp256k1_scalar *rr, secp256k1_scalar *rs,
                                     const unsigned char *sig, size_t size) {
  const unsigned char *sigend = sig + size;
  int rlen;
  if (sig == sigend || *(sig++) != 0x30) {
    /* The encoding doesn't start with a constructed sequence (X.690-0207
     * 8.9.1). */
    return 0;
  }
  rlen = secp256k1_der_read_len(&sig, sigend);
...
static int secp256k1_ecdsa_sig_serialize(unsigned char *sig, size_t *size,
...
static int secp256k1_ecdsa_sig_verify(const secp256k1_ecmult_context *ctx,
   ...
   *    <=> (xr == pr.x / pr.z^2 mod p) || (xr + n < p && xr + n == pr.x /
   * pr.z^2 mod p)
   *    [Multiplying both sides of the equations by pr.z^2 mod p]
   *    <=> (xr * pr.z^2 mod p == pr.x) || (xr + n < p && (xr + n) * pr.z^2 mod
   * p == pr.x)
   *
...
static int secp256k1_ecdsa_sig_sign(const secp256k1_ecmult_gen_context *ctx,
  ...
  if (recid) {
     ...
     * of some P where P.x >= order, and only 1 in about 2^127 points meet this
     * criteria.
     */

...
static void secp256k1_ecdsa_recoverable_signature_load(
    const secp256k1_context *ctx, secp256k1_scalar *r, secp256k1_scalar *s,
  ...
  if (sizeof(secp256k1_scalar) == 32) {
     ...
     * fast.
     * Note that secp256k1_ecdsa_signature_save must use the same
     * representation. */
...
static void secp256k1_ecdsa_recoverable_signature_save(
    secp256k1_ecdsa_recoverable_signature *sig, const secp256k1_scalar *r,
...
static int secp256k1_ecdsa_sig_recover(const secp256k1_ecmult_context *ctx,
  ...
  (void)r;
  VERIFY_CHECK(r); /* brx comes from a scalar, so is less than the order;
                      certainly less than p */
  if (recid & 2) {
...
int secp256k1_ecdsa_recover(
  ...
  VERIFY_CHECK(recid >= 0 &&
               recid < 4); /* should have been caught in parse_compact */

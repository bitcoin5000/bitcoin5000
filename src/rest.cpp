...
static bool rest_getutxos(HTTPRequest *req, const std::string &strURIPart) {
  ...
  case RF_BINARY: {
    try {
      ...
      if (strRequestMutable.size() > 0) {
        if (fInputParsed) // don't allow sending input over URI and HTTP RAW
                          // DATA
          return RESTERR(req, HTTP_BAD_REQUEST, "Combination of URI scheme "
                                                "inputs and raw post data is "
                                                "not allowed");
  ...
  // check spentness and form a bitmap (as well as a JSON capable human-readable
  ...
  bitmap.resize((vOutPoints.size() + 7) / 8);
  {
    ...
    if (fCheckMemPool)
      view.SetBackend(viewMempool); // switch cache backend to db+mempool in
                                    // case user likes to query mempool
    ...
    for (size_t i = 0; i < vOutPoints.size(); i++) {
      ...
      hits.push_back(hit);
      bitmapStringRepresentation.append(hit ? "1" : "0"); // form a binary
                                                          // string
                                                          // representation
                                                          // (human-readable for
                                                          // json output)
  ...
  case RF_JSON: {
    ...
    objGetUTXOResponse.push_back(
        Pair("chaintipHash", chainActive.Tip()->GetBlockHash().GetHex()));

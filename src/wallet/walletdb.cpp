...
bool ReadKeyValue(CWallet *pwallet, CDataStream &ssKey, CDataStream &ssValue,
  ...
  try {
    ...
    if (strType == "name") {
      ...
      ssKey >> strAddress;
      ssValue >>
          pwallet->mapAddressBook[CBitcoin5000Address(strAddress).Get()].name;
    } else if (strType == "purpose") {
      std::string strAddress;
      ssKey >> strAddress;
      ssValue >> pwallet->mapAddressBook[CBitcoin5000Address(strAddress).Get()]
                     .purpose;
    } else if (strType == "tx") {
      uint256 hash;
      ssKey >> hash;
      ...
      ssKey >> strKey;
      ssValue >> strValue;
      if (!pwallet->LoadDestData(CBitcoin5000Address(strAddress).Get(), strKey,
                                 strValue)) {

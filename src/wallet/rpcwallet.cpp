...
UniValue getnewaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 1)
    throw std::runtime_error(
        ...
        "\nReturns a new Bitcoin5000 address for receiving payments.\n"
        ...
        "\nResult:\n"
        "\"address\"    (string) The new bitcoin5000 address\n"
        "\nExamples:\n" +
  ...
  return CBitcoin5000Address(keyID).ToString();
}

CBitcoin5000Address GetAccountAddress(CWallet *const pwallet,
                                      std::string strAccount,
  ...
  return CBitcoin5000Address(pubKey.GetID());
}
...
UniValue getaccountaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nDEPRECATED. Returns the current Bitcoin5000 address for receiving "
        ...
        "given name.\n"
        "\nResult:\n"
        "\"address\"          (string) The account bitcoin5000 address\n"
        "\nExamples:\n" +
...
UniValue getrawchangeaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 0)
    throw std::runtime_error(
        "getrawchangeaddress\n"
        "\nReturns a new Bitcoin5000 address, for receiving change.\n"
...
UniValue setaccount(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"         (string, required) The bitcoin5000 address to "
  ...
  CBitcoin5000Address address(request.params[0].get_str());
  if (!address.IsValid())
    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                       "Invalid Bitcoin5000 address");
...
UniValue getaccount(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"         (string, required) The bitcoin5000 address for "
        "account lookup.\n"
        "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "  \"address\"         (string) a bitcoin5000 address associated with "
        "the given account\n"
        "  ,...\n"
        "]\n"
        "\nExamples:\n" +
  ...
  for (const std::pair<CBitcoin5000Address, CAddressBookData> &item :
       pwallet->mapAddressBook) {
    const CBitcoin5000Address &address = item.first;
...
UniValue sendtoaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"            (string, required) The bitcoin5000 address "
        "to send to.\n"
        ...
        "bitcoin5000s than you enter in the amount field.\n"
...
UniValue listaddressgroupings(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        "  [\n"
        "    [\n"
        "      \"address\",            (string) The bitcoin5000 address\n"
  ...
  for (std::set<CTxDestination> grouping : pwallet->GetAddressGroupings()) {
    ...
    for (CTxDestination address : grouping) {
      ...
      addressInfo.push_back(CBitcoin5000Address(address).ToString());
      addressInfo.push_back(ValueFromAmount(balances[address]));
      {
        if (pwallet->mapAddressBook.find(CBitcoin5000Address(address).Get()) !=
            pwallet->mapAddressBook.end()) {
          addressInfo.push_back(
              pwallet->mapAddressBook.find(CBitcoin5000Address(address).Get())
                  ->second.name);
        }
      }
...
UniValue signmessage(const JSONRPCRequest &request) {
  ...
  CBitcoin5000Address addr(strAddress);
  if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const JSONRPCRequest &request) {
  ...
  // Bitcoin5000 address
  CBitcoin5000Address address =
      CBitcoin5000Address(request.params[0].get_str());
  if (!address.IsValid())
    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                       "Invalid Bitcoin5000 address");
...
UniValue sendfrom(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
    throw std::runtime_error(
        ...
        "                       the spend.\n"
        "2. \"toaddress\"         (string, required) The bitcoin5000 address "
        "to send funds to.\n"
  ...
  std::string strAccount = AccountFromValue(request.params[0]);
  CBitcoin5000Address address(request.params[1].get_str());
  if (!address.IsValid())
    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                       "Invalid Bitcoin5000 address");
  CAmount nAmount = AmountFromValue(request.params[2]);
...
UniValue sendmany(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
    throw std::runtime_error(
        ...
        "    {\n"
        "      \"address\":amount   (numeric or string) The bitcoin5000 "
        ...
        "bitcoin5000s than you enter in their corresponding amount field.\n"
  ...
  std::set<CBitcoin5000Address> setAddress;
  std::vector<CRecipient> vecSend;

  CAmount totalAmount = 0;
  std::vector<std::string> keys = sendTo.getKeys();
  for (const std::string &name_ : keys) {
    CBitcoin5000Address address(name_);
    if (!address.IsValid())
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                         std::string("Invalid Bitcoin5000 address: ") + name_);
...
UniValue addmultisigaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 3) {
    std::string msg =
        ...
        "Each key is a Bitcoin5000 address or hex-encoded public key.\n"
        ...
        "2. \"keys\"         (string, required) A json array of bitcoin5000 "
        "addresses or hex-encoded public keys\n"
        "     [\n"
        "       \"address\"  (string) bitcoin5000 address or hex-encoded "
        "public key\n"
        "       ...,\n"
        "     ]\n"
  ...
  return CBitcoin5000Address(innerID).ToString();
}
...
UniValue addwitnessaddress(const JSONRPCRequest &request) {
  ...
  return CBitcoin5000Address(w.result).ToString();
}
...
UniValue ListReceived(CWallet *const pwallet, const UniValue &params,
  ...
  // Tally
  std::map<CBitcoin5000Address, tallyitem> mapTally;
  for (const std::pair<uint256, CWalletTx> &pairWtx : pwallet->mapWallet) {
    const CWalletTx &wtx = pairWtx.second;
  ...
  for (const std::pair<CBitcoin5000Address, CAddressBookData> &item :
    ...
    std::map<CBitcoin5000Address, tallyitem>::iterator it =
...
UniValue listtransactions(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 4)
    throw std::runtime_error(
        ...
        "                                                It will be \"\" for "
        "the default account.\n"
        "    \"address\":\"address\",    (string) The bitcoin5000 address of "
...
UniValue gettransaction(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "account.\n"
        "      \"address\" : \"address\",          (string) The bitcoin5000 "
...
UniValue walletpassphrase(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 2) {
    throw std::runtime_error(
        ...
        "keys such as sending bitcoin5000s\n"
        "\nArguments:\n"
...
UniValue encryptwallet(const JSONRPCRequest &request) {
  ...
  StartShutdown();
  return "wallet encrypted; Bitcoin5000 server stopping, restart to run with "
...
UniValue lockunspent(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "selection, when spending bitcoin5000s.\n"
...
UniValue listunspent(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 5)
    throw std::runtime_error(
        ...
        "confirmations to filter\n"
        "3. \"addresses\"      (string) A json array of bitcoin5000 addresses "
        "to filter\n"
        "    [\n"
        "      \"address\"     (string) bitcoin5000 address\n"
        "      ,...\n"
        "    ]\n"
        ...
        "    \"vout\" : n,               (numeric) the vout value\n"
        "    \"address\" : \"address\",    (string) the bitcoin5000 address\n"
  ...
  if (request.params.size() > 2 && !request.params[2].isNull()) {
    ...
    for (unsigned int idx = 0; idx < inputs.size(); idx++) {
      ...
      CBitcoin5000Address address(input.get_str());
      if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                           std::string("Invalid Bitcoin5000 address: ") +
                               input.get_str());
      if (setAddress.count(address))
        throw JSONRPCError(
            RPC_INVALID_PARAMETER,
  ...
  for (const COutput &out : vecOutputs) {
    ...
    if (fValidAddress) {
      entry.push_back(Pair("address", CBitcoin5000Address(address).ToString()));
...
UniValue fundrawtransaction(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "address) The bitcoin5000 address to receive the change\n"
  ...
  if (!request.params[1].isNull()) {
    if (request.params[1].type() == UniValue::VBOOL) {
      ...
      if (options.exists("changeAddress")) {
        CBitcoin5000Address address(options["changeAddress"].get_str());

        if (!address.IsValid())
          throw JSONRPCError(
              RPC_INVALID_ADDRESS_OR_KEY,
              "changeAddress must be a valid bitcoin5000 address");
...
UniValue getgenerate(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 0) {
    throw std::runtime_error(
        ...
        std::string(BITCOIN5000_CONF_FILENAME) +
        " setting gen)\n"
...
UniValue setgenerate(const JSONRPCRequest &request) {
  ...
  GenerateBitcoin5000s(fGenerate, nGenProcLimit, Params());

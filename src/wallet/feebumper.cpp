...
int64_t CalculateMaximumSignedTxSize(const CTransaction &tx,
  ...
  for (auto &input : tx.vin) {
    ...
    assert(mi != pWallet->mapWallet.end() &&
...
bool CFeeBumper::preconditionChecks(const CWallet *pWallet,
  ...
  if (wtx.GetDepthInMainChain() != 0) {
    vErrors.push_back("Transaction has been mined, or is conflicted with a "
                      "mined transaction");
...
CFeeBumper::CFeeBumper(const CWallet *pWallet, const uint256 txidIn,
  ...
  // if not, we can't bump the fee, because the wallet has no way of knowing the
  // value of the other inputs (thus the fee)
  ...
  if (totalFee > 0) {
    ...
    nNewFee =
        CWallet::GetMinimumFee(maxNewTxSize, coin_control, mempool,
                               ::feeEstimator, nullptr /* FeeCalculation */);
    nNewFeeRate = CFeeRate(nNewFee, maxNewTxSize);

    // New fee rate must be at least old rate + minimum incremental relay rate
    // walletIncrementalRelayFee.GetFeePerK() should be exact, because it's
    // initialized
  ...
  if (nNewFee > maxTxFee) {
    vErrors.push_back(strprintf("Specified or calculated fee %s is too high "
  ...
  // (no point in bumping fee if we know that the new tx won't be accepted to
  // the mempool)
  // This may occur if the user set TotalFee or paytxfee too low, if fallbackfee
  // is too low, or, perhaps,
  // in a rare situation where the mempool minimum fee increased significantly
  // since the fee estimation just a
  // moment earlier. In this case, we report an error to the user, who may use
  // totalFee to make an adjustment.
  CFeeRate minMempoolFeeRate = mempool.GetMinFee(
      gArgs.GetArg("-maxmempool", DEFAULT_MAX_MEMPOOL_SIZE) * 1000000);
  if (nNewFeeRate.GetFeePerK() < minMempoolFeeRate.GetFeePerK()) {
    vErrors.push_back(strprintf(
        "New fee rate (%s) is less than the minimum fee rate (%s) to get into "
        "the mempool. totalFee value should to be at least %s or settxfee "
...
bool CFeeBumper::commit(CWallet *pWallet) {
  ...
  // make sure the transaction still has no descendants and hasn't been mined in
  // the meantime
  ...
  if (!pWallet->CommitTransaction(wtxBumped, reservekey, g_connman.get(),
                                  state)) {
  ...
  if (state.IsInvalid()) {
    ...
    vErrors.push_back(strprintf("Error: The transaction was rejected: %s",
  ...
  if (!pWallet->MarkReplaced(oldWtx.GetHash(), wtxBumped.GetHash())) {
    ...
    vErrors.push_back("Error: Created new bumpfee transaction but could not "

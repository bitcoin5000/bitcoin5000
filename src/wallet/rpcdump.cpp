...
void ImportAddress(CWallet *, const CBitcoin5000Address &address,
...
void ImportScript(CWallet *const pwallet, const CScript &script,
  ...
  if (isRedeemScript) {
    ...
    ImportAddress(pwallet, CBitcoin5000Address(CScriptID(script)), strLabel);
  } else {
...
void ImportAddress(CWallet *const pwallet, const CBitcoin5000Address &address,
                   const std::string &strLabel) {
...
UniValue importaddress(const JSONRPCRequest &request) {
  ...
  if (address.IsValid()) {
    ...
    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                       "Invalid Bitcoin5000 address or script");
  }
...
UniValue importpubkey(const JSONRPCRequest &request) {
  ...
  ImportAddress(pwallet, CBitcoin5000Address(pubKey.GetID()), strLabel);
  ...
  while (file.good()) {
    ...
    if (pwallet->HaveKey(keyid)) {
      LogPrintf("Skipping import of %s (key already present)\n",
                CBitcoin5000Address(keyid).ToString());
      continue;
    }
...
UniValue dumpprivkey(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"   (string, required) The bitcoin5000 address for the "
        "private key\n"
        "\nResult:\n"
  ...
  return CBitcoin5000Secret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "absolute or relative to bitcoin5000d)\n"
        "\nResult:\n"
        "{                           (json object)\n"
   ...
   * https://github.com/bitcoin5000/bitcoin5000/issues/9934
  ...
  // produce output
  file << strprintf("# Wallet dump created by Bitcoin5000 %s\n", CLIENT_BUILD);
  ...
  for (std::vector<std::pair<int64_t, CKeyID>>::const_iterator it =
    ...
    std::string strAddr = CBitcoin5000Address(keyid).ToString();
    CKey key;
    if (pwallet->GetKey(keyid, key)) {
      file << strprintf("%s %s ", CBitcoin5000Secret(key).ToString(), strTime);
...
UniValue ProcessImport(CWallet *const pwallet, const UniValue &data,
  ...
  try {
    ...
    if (!isScript) {
      address = CBitcoin5000Address(output);
      if (!address.IsValid()) {
    ...
    if (isP2SH) {
      ...
      CBitcoin5000Address redeemAddress =
          CBitcoin5000Address(CScriptID(redeemScript));
      ...
      if (pubKeys.size() && keys.size() == 0) {
        ...
        CBitcoin5000Address pubKeyAddress = CBitcoin5000Address(pubKey.GetID());
        ...
        // Consistency check.
        if (isScript) {
          CBitcoin5000Address scriptAddress;
          CTxDestination destination;

          if (ExtractDestination(script, destination)) {
            scriptAddress = CBitcoin5000Address(destination);
          ...
          result.pushKV(
              ...
              JSONRPCError(
                  ...
                  strprintf("Rescan failed for key with creation timestamp %d. "
                            ...
                            "bitcoin5000d log for details) and could "

...
bool CWallet::LoadCScript(const CScript &redeemScript) {
  ...
  if (redeemScript.size() > MAX_SCRIPT_ELEMENT_SIZE) {
    std::string strAddr =
        CBitcoin5000Address(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const {
  ...
  // https://github.com/bitcoin5000/bitcoin5000/pull/10340, but that change has
  // been
  // postponed until after https://github.com/bitcoin5000/bitcoin5000/pull/10244
  // to
...
bool CWallet::CreateTransaction(const std::vector<CRecipient> &vecSend,
  ...
  unsigned int nBytes;
  {
    ...
    LOCK2(cs_main, cs_wallet);
    {
      ...
      // change transaction isn't always pay-to-bitcoin5000-address
...
bool CWallet::SetAddressBook(const CTxDestination &address,
  ...
  if (!strPurpose.empty() &&
      !CWalletDB(*dbw).WritePurpose(CBitcoin5000Address(address).ToString(),
                                    strPurpose))
    return false;
  return CWalletDB(*dbw).WriteName(CBitcoin5000Address(address).ToString(),
                                   strName);
}
...
bool CWallet::DelAddressBook(const CTxDestination &address) {
  {
    ...
    std::string strAddress = CBitcoin5000Address(address).ToString();
  ...
  CWalletDB(*dbw).ErasePurpose(CBitcoin5000Address(address).ToString());
  return CWalletDB(*dbw).EraseName(CBitcoin5000Address(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key,
  ...
  return CWalletDB(*dbw).WriteDestData(CBitcoin5000Address(dest).ToString(),
                                       key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest,
  ...
  if (!mapAddressBook[dest].destdata.erase(key))
    return false;
  return CWalletDB(*dbw).EraseDestData(CBitcoin5000Address(dest).ToString(),
                                       key);
}

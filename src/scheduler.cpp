...
toPosixTime(const boost::chrono::system_clock::time_point &t) {
  // Creating the posix_time using from_time_t loses sub-second precision. So
  // rather than exporting the time_point to time_t,
  // start with a posix_time at the epoch (0) and add the milliseconds that have
  // passed since then.
...
// wait_until needs boost 1.50 or later; older versions have timed_wait:
...
#else
      // Some boost versions have a conflicting overload of wait_until that
      // returns void.
      ...
      while (!shouldStop() && !taskQueue.empty()) {
        ...
        if (newTaskScheduled.wait_until<>(lock, timeToWaitFor) ==
          ...
          break; // Exit loop after timeout, it means we reached the time of the
                 // event
      }
#endif

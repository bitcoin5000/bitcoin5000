...
CBloomFilter::CBloomFilter(const unsigned int nElements, const double nFPRate,
    ...
    : /**
       * The ideal size for a bloom filter with a given number of elements and
       * false positive rate is:
       * - nElements * log(fp rate) / ln(2)^2
       * We ignore filter parameters which will create a bloom filter larger
       * than the protocol limits
       */
       ...
       * elements
       * Again, we ignore filter parameters which will create a bloom filter
       * with more hash functions than the protocol limits
       * See https://en.wikipedia.org/wiki/Bloom_filter for an explanation of
       * these formulas
       */
...
bool CBloomFilter::IsRelevantAndUpdate(const CTransaction &tx) {
  ...
  for (unsigned int i = 0; i < tx.vout.size(); i++) {
    ...
    // This means clients don't have to update the filter themselves when a new
    // relevant tx
    // is discovered in order to find spending transactions, which avoids
...
void CRollingBloomFilter::insert(const std::vector<unsigned char> &vKey) {
  ...
  for (int n = 0; n < nHashFuncs; n++) {
    ...
    /* The lowest bit of pos is ignored, and set to zero for the first bit, and
     * to one for the second. */
...
bool CRollingBloomFilter::contains(
  ...
  for (int n = 0; n < nHashFuncs; n++) {
    ...
    int bit = h & 0x3F;
    uint32_t pos = (h >> 6) % data.size();
    /* If the relevant bit is not set in either data[pos & ~1] or data[pos | 1],
     * the filter does not contain vKey */

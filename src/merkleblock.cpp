...
uint256 CPartialMerkleTree::CalcHash(int height, unsigned int pos,
  ...
  // we can never have zero txs in a merkle block, we always need the coinbase
  // tx
  // if we do not have this assert, we can hit a memory access violation when
  // indexing into vTxid
  assert(vTxid.size() != 0);
  if (height == 0) {
...
uint256 CPartialMerkleTree::TraverseAndExtract(
  ...
  if (height == 0 || !fParentOfMatch) {
    // if at height 0, or nothing interesting below, use stored hash and do not
    // descend
    ...
    uint256 left = TraverseAndExtract(height - 1, pos * 2, nBitsUsed, nHashUsed,
            ...
            right;
    if (pos * 2 + 1 < CalcTreeWidth(height - 1)) {
      right = TraverseAndExtract(height - 1, pos * 2 + 1, nBitsUsed, nHashUsed,
                                 vMatch, vnIndex);
      if (right == left) {

...
static bool ProcessBlockFound(const std::shared_ptr<const CBlock> pblock,
  ...
  // Found a solution
  {
    ...
    if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
      return error("Bitcoin5000Miner: generated block is stale");
  }
  ...
  if (!ProcessNewBlock(chainparams, pblock, true, NULL))
    return error("Bitcoin5000Miner: ProcessNewBlock, block not accepted");

  return true;
}

void static Bitcoin5000Miner(const CChainParams &chainparams) {
  LogPrintf("Bitcoin5000Miner started\n");
  // SetThreadPriority(THREAD_PRIORITY_LOWEST);
  RenameThread("bitcoin5000-miner");
  ...
  try {
    ...
    while (true) {
      ...
      if (!pblocktemplate.get()) {
        LogPrintf("Error in Bitcoin5000Miner: Keypool ran out, please call "
      ...
      // LogPrintf("Running Bitcoin5000Miner with %u transactions in block (%u
      // bytes)\n", pblock->vtx.size(),
    ...
    LogPrintf("Bitcoin5000Miner terminated\n");
    throw;
  } catch (const std::runtime_error &e) {
    LogPrintf("Bitcoin5000Miner runtime error: %s\n", e.what());
    return;
  }
}

void GenerateBitcoin5000s(bool fGenerate, int nThreads,
                          const CChainParams &chainparams) {
  ...
  for (int i = 0; i < nThreads; i++)
    minerThreads->create_thread(
        boost::bind(&Bitcoin5000Miner, boost::cref(chainparams)));
}

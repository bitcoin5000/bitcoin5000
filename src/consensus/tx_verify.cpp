...
std::pair<int, int64_t> CalculateSequenceLocks(const CTransaction &tx,
  ...
  for (size_t txinIndex = 0; txinIndex < tx.vin.size(); txinIndex++) {
    ...
    if (txin.nSequence & CTxIn::SEQUENCE_LOCKTIME_TYPE_FLAG) {
      ...
      nMinTime = std::max(
          nMinTime,
          nCoinTime + (int64_t)((txin.nSequence & CTxIn::SEQUENCE_LOCKTIME_MASK)
                                << CTxIn::SEQUENCE_LOCKTIME_GRANULARITY) -
              1);
    } else {
      nMinHeight = std::max(
          nMinHeight,
          nCoinHeight + (int)(txin.nSequence & CTxIn::SEQUENCE_LOCKTIME_MASK) -
              1);
    }
  }
...
bool CheckTransaction(const CTransaction &tx, CValidationState &state,
  ...
  // Size limits (this doesn't take the witness into account, as that hasn't
  // been checked for malleability)
  if (::GetSerializeSize(tx, SER_NETWORK,
  ...
  // Check for duplicate inputs - note that this check is slow so we skip it in
  // CheckBlock
  ...
  if (tx.IsCoinBase()) {
    if (tx.vin[0].scriptSig.size() < 2 || tx.vin[0].scriptSig.size() > 132)

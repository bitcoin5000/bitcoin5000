...
Intro::Intro(QWidget *parent)
  ...
  ui->sizeWarningLabel->setText(
      tr("%1 will download and store a copy of the Bitcoin5000 block chain.")
          .arg(tr(PACKAGE_NAME)) +
...
bool Intro::pickDataDirectory() {
  ...
  if (!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) ||
    ...
    intro.setWindowIcon(QIcon(":icons/bitcoin5000"));
   ...
   * to
   * override -datadir in the bitcoin5000.conf file in the default data
   * directory
   * (to be consistent with bitcoin5000d behavior)
   */
  if (dataDir != getDefaultDataDirectory())

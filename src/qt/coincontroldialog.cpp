...
// click on header
  ...
  // see https://github.com/bitcoin5000/bitcoin5000/issues/5716
...
void CoinControlDialog::copyAmount() {
  GUIUtil::setClipboard(
      Bitcoin5000Units::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog) {
  ...
  int nDisplayUnit = Bitcoin5000Units::BVK;
  ...
  l2->setText(
      Bitcoin5000Units::formatWithUnit(nDisplayUnit, nAmount)); // Amount
  l3->setText(Bitcoin5000Units::formatWithUnit(nDisplayUnit, nPayFee)); // Fee
  l4->setText(
      Bitcoin5000Units::formatWithUnit(nDisplayUnit, nAfterFee)); // After Fee
  l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") +
              QString::number(nBytes));      // Bytes
  l7->setText(fDust ? tr("yes") : tr("no")); // Dust
  l8->setText(
      Bitcoin5000Units::formatWithUnit(nDisplayUnit, nChange)); // Change
  if (nPayFee > 0) {
  ...
  for (const std::pair<QString, std::vector<COutput>> &coins : mapCoins) {
    ...
    for (const COutput &out : coins.second) {
      ...
      if (ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey,
        ...
        sAddress = QString::fromStdString(
            CBitcoin5000Address(outputAddress).ToString());

        // if listMode or change => show bitcoin5000 address. In tree mode,
    ...
    if (treeMode) {
      ...
      itemWalletAddress->setText(COLUMN_AMOUNT,
                                 Bitcoin5000Units::format(nDisplayUnit, nSum));

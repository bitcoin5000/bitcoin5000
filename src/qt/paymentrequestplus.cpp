...
bool PaymentRequestPlus::getMerchant(X509_STORE *certStore,
  ...
  if (paymentRequest.pki_type() == "x509+sha256") {
    ...
    qWarning()
        << "PaymentRequestPlus::getMerchant: Payment request: unknown pki_type "
  ...
  try {
    ...
    if (result != 1) {
      ...
      if (!(error == X509_V_ERR_DEPTH_ZERO_SELF_SIGNED_CERT &&
        ...
        qDebug() << "PaymentRequestPlus::getMerchant: Allowing self signed "
                    "root certificate, because "
    ...
    // TODO: detect EV certificates and set merchant = business name instead of
    // unfriendly NID_commonName ?

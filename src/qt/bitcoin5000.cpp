...
static std::string Translate(const char *psz) {
  return QCoreApplication::translate("bitcoin5000-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase,
  ...
  // Load e.g. bitcoin5000_de.qm (shortcut "de" needs to be defined in
  // bitcoin5000.qrc)
  if (translatorBase.load(lang, ":/translations/"))
    QApplication::installTranslator(&translatorBase);

  // Load e.g. bitcoin5000_de_DE.qm (shortcut "de_DE" needs to be defined in
  // bitcoin5000.qrc)
...
/** Class encapsulating Bitcoin5000 Core startup and shutdown.
 * Allows running startup and shutdown in a different thread from the UI thread.
 */
class Bitcoin5000Core : public QObject {
  Q_OBJECT
public:
  explicit Bitcoin5000Core();
...
/** Main Bitcoin5000 application object */
class Bitcoin5000Application : public QApplication {
  Q_OBJECT
public:
  explicit Bitcoin5000Application(int &argc, char **argv);
  ~Bitcoin5000Application();
  ...
  /// Get window identifier of QMainWindow (Bitcoin5000GUI)
...
#include "bitcoin5000.moc"

Bitcoin5000Core::Bitcoin5000Core() : QObject() {}

void Bitcoin5000Core::handleRunawayException(const std::exception *e) {
  PrintExceptionContinue(e, "Runaway exception");
  Q_EMIT runawayException(QString::fromStdString(GetWarnings("gui")));
}

bool Bitcoin5000Core::baseInitialize() {
...
void Bitcoin5000Core::initialize() {
  try {
...
Bitcoin5000Application::Bitcoin5000Application(int &argc, char **argv)
  ...
  // This must be done inside the Bitcoin5000Application constructor, or after
  // it, because
  // PlatformStyle::instantiate requires a QApplication
  std::string platformName;
  platformName =
      gArgs.GetArg("-uiplatform", Bitcoin5000GUI::DEFAULT_UIPLATFORM);
  platformStyle =
...
Bitcoin5000Application::~Bitcoin5000Application() {
  if (coreThread) {
...
#ifdef ENABLE_WALLET
void Bitcoin5000Application::createPaymentServer() {
  paymentServer = new PaymentServer(this);
}
#endif

void Bitcoin5000Application::createOptionsModel(bool resetSettings) {
  optionsModel = new OptionsModel(nullptr, resetSettings);
}

void Bitcoin5000Application::createWindow(const NetworkStyle *networkStyle) {
  window = new Bitcoin5000GUI(platformStyle, networkStyle, 0);
...
void Bitcoin5000Application::createSplashScreen(
...
void Bitcoin5000Application::startThread() {
  if (coreThread)
    return;
  coreThread = new QThread(this);
  Bitcoin5000Core *executor = new Bitcoin5000Core();
...
void Bitcoin5000Application::parameterSetup() {
  InitLogging();
  InitParameterInteraction();
}

void Bitcoin5000Application::requestInitialize() {
  qDebug() << __func__ << ": Requesting initialize";
  startThread();
  Q_EMIT requestedInitialize();
}

void Bitcoin5000Application::requestShutdown() {
...
void Bitcoin5000Application::initializeResult(bool success) {
  ...
  if (success) {
    ...
    if (!vpwallets.empty()) {
      ...
      window->addWallet(Bitcoin5000GUI::DEFAULT_WALLET, walletModel);
      window->setCurrentWallet(Bitcoin5000GUI::DEFAULT_WALLET);
    ...
    // bitcoin5000: URIs or payment requests:
...
void Bitcoin5000Application::shutdownResult() {
  quit(); // Exit main loop after shutdown finished
}

void Bitcoin5000Application::handleRunawayException(const QString &message) {
  QMessageBox::critical(
      0, "Runaway exception",
      Bitcoin5000GUI::tr("A fatal error occurred. Bitcoin5000 can no longer "
                         "continue safely and will quit.") +
          QString("\n\n") + message);
  ::exit(EXIT_FAILURE);
}

WId Bitcoin5000Application::getMainWinId() const {
  if (!window)
    return 0;
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
  ...
  Q_INIT_RESOURCE(bitcoin5000);
  Q_INIT_RESOURCE(bitcoin5000_locale);

  Bitcoin5000Application app(argc, argv);
  ...
  /// 6. Determine availability of data directory and parse bitcoin5000.conf
  ...
  try {
    gArgs.ReadConfigFile(gArgs.GetArg("-conf", BITCOIN5000_CONF_FILENAME));
  } catch (const std::exception &e) {
  ...
  // bitcoin5000: links repeatedly have their payment requests routed to this
  // process:
  ...
  try {
    ...
    if (Bitcoin5000Core::baseInitialize()) {

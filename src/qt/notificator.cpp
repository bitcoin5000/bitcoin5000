...
#ifdef Q_OS_MAC
void Notificator::notifyGrowl(Class cls, const QString &title,
                              const QString &text, const QIcon &icon) {
  const QString script(
      ...
      "  set the enabledNotificationsList to {\"Notification\"}\n" // -- Make a
                                                                   ...
                                                                   // notifications
                                                                   // (enabled)
      "  register as application \"%1\" all notifications allNotificationsList "
      "default notifications enabledNotificationsList\n" // -- Register our
                                                         // script with Growl
      "  notify with name \"Notification\" title \"%2\" description \"%3\" "
      "application name \"%1\"%4\n" // -- Send a Notification
      "end tell");
...
void Notificator::notifyMacUserNotificationCenter(Class cls,
                                                  ...
                                                  const QString &text,
                                                  const QIcon &icon) {
  // icon is not supported by the user notification center yet. OSX will use the
  // app icon.
...
void Notificator::notify(Class cls, const QString &title, const QString &text,
  ...
  default:
    if (cls == Critical) {
      // Fall back to old fashioned pop-up dialog if critical and no other

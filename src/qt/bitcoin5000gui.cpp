...
#include "config/bitcoin5000-config.h"
#endif

#include "bitcoin5000gui.h"
...
const std::string Bitcoin5000GUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
    "macosx"
...
const QString Bitcoin5000GUI::DEFAULT_WALLET = "~Default";

Bitcoin5000GUI::Bitcoin5000GUI(const PlatformStyle *_platformStyle,
...
void Bitcoin5000GUI::createActions() {
  ...
  sendCoinsAction = new QAction(platformStyle->SingleColorIcon(":/icons/send"),
                                tr("&Send"), this);
  sendCoinsAction->setStatusTip(tr("Send coins to a Bitcoin5000 address"));
  ...
  receiveCoinsAction->setStatusTip(
      tr("Request payments (generates QR codes and bitcoin5000: URIs)"));
  ...
  signMessageAction->setStatusTip(tr(
      "Sign messages with your Bitcoin5000 addresses to prove you own them"));
  verifyMessageAction =
  ...
  openAction = new QAction(platformStyle->TextColorIcon(":/icons/open"),
                           tr("Open &URI..."), this);
  openAction->setStatusTip(tr("Open a bitcoin5000: URI or payment request"));
  ...
  showHelpMessageAction->setStatusTip(
      tr("Show the %1 help message to get a list with possible Bitcoin5000 "
...
void Bitcoin5000GUI::createMenuBar() {
#ifdef Q_OS_MAC
...
void Bitcoin5000GUI::createToolBars() {
  if (walletFrame) {
...
void Bitcoin5000GUI::setClientModel(ClientModel *_clientModel) {
...
#ifdef ENABLE_WALLET
bool Bitcoin5000GUI::addWallet(const QString &name, WalletModel *walletModel) {
  if (!walletFrame)
    return false;
...
bool Bitcoin5000GUI::setCurrentWallet(const QString &name) {
  if (!walletFrame)
    return false;
  return walletFrame->setCurrentWallet(name);
}

void Bitcoin5000GUI::removeAllWallets() {
  if (!walletFrame)
    return;
  setWalletActionsEnabled(false);
...
void Bitcoin5000GUI::setWalletActionsEnabled(bool enabled) {
...
void Bitcoin5000GUI::createTrayIcon(const NetworkStyle *networkStyle) {
#ifndef Q_OS_MAC
...
void Bitcoin5000GUI::createTrayIconMenu() {
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void Bitcoin5000GUI::trayIconActivated(
...
void Bitcoin5000GUI::optionsClicked() {
...
void Bitcoin5000GUI::aboutClicked() {
  if (!clientModel)
    return;
...
void Bitcoin5000GUI::showDebugWindow() {
...
void Bitcoin5000GUI::showDebugWindowActivateConsole() {
  rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
  showDebugWindow();
}

void Bitcoin5000GUI::showHelpMessageClicked() { helpMessageDialog->show(); }

#ifdef ENABLE_WALLET
void Bitcoin5000GUI::openClicked() {
...
void Bitcoin5000GUI::gotoOverviewPage() {
  overviewAction->setChecked(true);
  if (walletFrame)
    walletFrame->gotoOverviewPage();
}

void Bitcoin5000GUI::gotoHistoryPage() {
  historyAction->setChecked(true);
  if (walletFrame)
    walletFrame->gotoHistoryPage();
}

void Bitcoin5000GUI::gotoReceiveCoinsPage() {
  receiveCoinsAction->setChecked(true);
  if (walletFrame)
    walletFrame->gotoReceiveCoinsPage();
}

void Bitcoin5000GUI::gotoSendCoinsPage(QString addr) {
  sendCoinsAction->setChecked(true);
  if (walletFrame)
    walletFrame->gotoSendCoinsPage(addr);
}

void Bitcoin5000GUI::gotoSignMessageTab(QString addr) {
  if (walletFrame)
    walletFrame->gotoSignMessageTab(addr);
}

void Bitcoin5000GUI::gotoVerifyMessageTab(QString addr) {
  if (walletFrame)
    walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void Bitcoin5000GUI::updateNetworkState() {
  ...
  if (clientModel->getNetworkActive()) {
    tooltip = tr("%n active connection(s) to Bitcoin5000 network", "", count) +
...
void Bitcoin5000GUI::setNumConnections(int count) { updateNetworkState(); }

void Bitcoin5000GUI::setNetworkActive(bool networkActive) {
  updateNetworkState();
}

void Bitcoin5000GUI::updateHeadersSyncProgressLabel() {
...
void Bitcoin5000GUI::setNumBlocks(int count, const QDateTime &blockDate,
...
void Bitcoin5000GUI::message(const QString &title, const QString &message,
                             unsigned int style, bool *ret) {
  QString strTitle = tr("Bitcoin5000"); // default title
...
void Bitcoin5000GUI::changeEvent(QEvent *e) {
...
void Bitcoin5000GUI::closeEvent(QCloseEvent *event) {
#ifndef Q_OS_MAC // Ignored on Mac
  if (clientModel && clientModel->getOptionsModel()) {
...
void Bitcoin5000GUI::showEvent(QShowEvent *event) {
...
#ifdef ENABLE_WALLET
void Bitcoin5000GUI::incomingTransaction(const QString &date, int unit,
  ...
  QString msg = tr("Date: %1\n").arg(date) +
                tr("Amount: %1\n")
                    .arg(Bitcoin5000Units::formatWithUnit(unit, amount, true)) +
                tr("Type: %1\n").arg(type);
  if (!label.isEmpty())
    msg += tr("Label: %1\n").arg(label);
...
void Bitcoin5000GUI::dragEnterEvent(QDragEnterEvent *event) {
  // Accept only URIs
  if (event->mimeData()->hasUrls())
    event->acceptProposedAction();
}

void Bitcoin5000GUI::dropEvent(QDropEvent *event) {
  if (event->mimeData()->hasUrls()) {
...
bool Bitcoin5000GUI::eventFilter(QObject *object, QEvent *event) {
...
#ifdef ENABLE_WALLET
bool Bitcoin5000GUI::handlePaymentRequest(const SendCoinsRecipient &recipient) {
  // URI has to be valid
...
void Bitcoin5000GUI::setHDStatus(int hdEnabled) {
...
void Bitcoin5000GUI::setEncryptionStatus(int status) {
  switch (status) {
...
void Bitcoin5000GUI::showNormalIfMinimized(bool fToggleHidden) {
  if (!clientModel)
    return;
...
void Bitcoin5000GUI::toggleHidden() { showNormalIfMinimized(true); }

void Bitcoin5000GUI::detectShutdown() {
...
void Bitcoin5000GUI::showProgress(const QString &title, int nProgress) {
  if (nProgress == 0) {
...
void Bitcoin5000GUI::setTrayIconVisible(bool fHideTrayIcon) {
  if (trayIcon) {
    trayIcon->setVisible(!fHideTrayIcon);
  }
}

void Bitcoin5000GUI::showModalOverlay() {
  if (modalOverlay &&
      (progressBar->isVisible() || modalOverlay->isLayerVisible()))
    modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(Bitcoin5000GUI *gui,
...
void Bitcoin5000GUI::subscribeToCoreSignals() {
...
void Bitcoin5000GUI::unsubscribeFromCoreSignals() {
...
void Bitcoin5000GUI::toggleNetworkActive() {
  if (clientModel) {
...
UnitDisplayStatusBarControl::UnitDisplayStatusBarControl(
  ...
  QList<Bitcoin5000Units::Unit> units = Bitcoin5000Units::availableUnits();
  int max_width = 0;
  const QFontMetrics fm(font());
  for (const Bitcoin5000Units::Unit unit : units) {
    max_width = qMax(max_width, fm.width(Bitcoin5000Units::name(unit)));
  }
...
void UnitDisplayStatusBarControl::createContextMenu() {
  menu = new QMenu(this);
  for (Bitcoin5000Units::Unit u : Bitcoin5000Units::availableUnits()) {
    QAction *menuAction = new QAction(QString(Bitcoin5000Units::name(u)), this);
...
void UnitDisplayStatusBarControl::updateDisplayUnit(int newUnits) {
  setText(Bitcoin5000Units::name(newUnits));
}

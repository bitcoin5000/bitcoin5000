...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx,
  ...
  if (wtx.IsCoinBase()) {
    ...
    if (nNet > 0) {
      // Credit
      if (CBitcoin5000Address(rec->address).IsValid()) {
        CTxDestination address = CBitcoin5000Address(rec->address).Get();
  ...
  if (wtx.mapValue.count("to") && !wtx.mapValue["to"].empty()) {
    ...
    strHTML += "<b>" + tr("To") + ":</b> ";
    CTxDestination dest = CBitcoin5000Address(strAddress).Get();
  ...
  if (wtx.IsCoinBase() && nCredit == 0) {
    ...
    if (wtx.IsInMainChain())
      strHTML +=
          Bitcoin5000Units::formatHtmlWithUnit(unit, nUnmatured) + " (" +
    ...
    //
    strHTML += "<b>" + tr("Credit") + ":</b> " +
               Bitcoin5000Units::formatHtmlWithUnit(unit, nNet) + "<br>";
  } else {
    ...
    if (fAllFromMe) {
      ...
      for (const CTxOut &txout : wtx.tx->vout) {
          ...
          if (ExtractDestination(txout.scriptPubKey, address)) {
            ...
            if (wallet->mapAddressBook.count(address) &&
              ...
              strHTML +=
                  ...
                  " ";
            strHTML +=
                GUIUtil::HtmlEscape(CBitcoin5000Address(address).ToString());
        ...
        strHTML += "<b>" + tr("Debit") + ":</b> " +
                   Bitcoin5000Units::formatHtmlWithUnit(unit, -txout.nValue) +
                   "<br>";
        if (toSelf)
          strHTML += "<b>" + tr("Credit") + ":</b> " +
                     Bitcoin5000Units::formatHtmlWithUnit(unit, txout.nValue) +
      ...
      if (fAllToMe) {
        ...
        strHTML += "<b>" + tr("Total debit") + ":</b> " +
                   Bitcoin5000Units::formatHtmlWithUnit(unit, -nValue) + "<br>";
      ...
      if (nTxFee > 0)
        strHTML += "<b>" + tr("Transaction fee") + ":</b> " +
                   Bitcoin5000Units::formatHtmlWithUnit(unit, -nTxFee) + "<br>";
    } else {
      //
      ...
      for (const CTxIn &txin : wtx.tx->vin)
        if (wallet->IsMine(txin))
          strHTML += "<b>" + tr("Debit") + ":</b> " +
                     Bitcoin5000Units::formatHtmlWithUnit(
  ...
  strHTML += "<b>" + tr("Net amount") + ":</b> " +
             Bitcoin5000Units::formatHtmlWithUnit(unit, nNet, true) + "<br>";
  ...
  // Message from normal bitcoin5000:URI (bitcoin5000:123...?message=example)
          ...
          if (ExtractDestination(vout.scriptPubKey, address)) {
            if (wallet->mapAddressBook.count(address) &&
              ...
              strHTML +=
                  ...
                  " ";
            strHTML +=
                QString::fromStdString(CBitcoin5000Address(address).ToString());
          }
          strHTML = strHTML + " " + tr("Amount") + "=" +
                    Bitcoin5000Units::formatHtmlWithUnit(unit, vout.nValue);

...
bool WalletModel::validateAddress(const QString &address) {
  CBitcoin5000Address addressParsed(address.toStdString());
...
WalletModel::prepareTransaction(WalletModelTransaction &transaction,
  ...
  for (const SendCoinsRecipient &rcp : recipients) {
    ...
    if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
      ...
      total += subtotal;
    } else { // User-entered bitcoin5000 address / amount:
      ...
      CScript scriptPubKey = GetScriptForDestination(
          CBitcoin5000Address(rcp.address.toStdString()).Get());
...
WalletModel::sendCoins(WalletModelTransaction &transaction) {
...

  {
    ...
    for (const SendCoinsRecipient &rcp : transaction.getRecipients()) {
      ...
      } else if (!rcp.message.isEmpty()) // Message from normal bitcoin5000:URI
                                         // (bitcoin5000:123...?message=example)
...
void WalletModel::listCoins(
  ...
  for (auto &group : wallet->ListCoins()) {
    auto &resultGroup = mapCoins[QString::fromStdString(
        CBitcoin5000Address(group.first).ToString())];
...
bool WalletModel::saveReceiveRequest(const std::string &sAddress,
  ...
  CTxDestination dest = CBitcoin5000Address(sAddress).Get();
...
bool WalletModel::bumpFee(uint256 hash) {
  ...
  questionString.append(Bitcoin5000Units::formatHtmlWithUnit(

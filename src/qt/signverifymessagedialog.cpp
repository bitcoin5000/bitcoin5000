...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked() {
  ...
  CBitcoin5000Address addr(ui->addressIn_SM->text().toStdString());
  if (!addr.IsValid()) {
...
void SignVerifyMessageDialog::on_verifyMessageButton_VM_clicked() {
  CBitcoin5000Address addr(ui->addressIn_VM->text().toStdString());
  if (!addr.IsValid()) {
  ...
  if (!(CBitcoin5000Address(pubkey.GetID()) == addr)) {
    ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");

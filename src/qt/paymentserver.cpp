...
const int BITCOIN5000_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString BITCOIN5000_IPC_PREFIX("bitcoin5000:");
// BIP70 payment protocol messages
const char *BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char *BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char *BIP71_MIMETYPE_PAYMENT = "application/bitcoin5000-payment";
const char *BIP71_MIMETYPE_PAYMENTACK = "application/bitcoin5000-paymentack";
const char *BIP71_MIMETYPE_PAYMENTREQUEST =
    "application/bitcoin5000-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char *argv[]) {
  for (int i = 1; i < argc; i++) {
    ...
    // If the bitcoin5000: URI contains a payment request, we are not able to
    // detect the
    // network as that would require fetching and parsing the payment request.
    // That means clicking such an URI which contains a testnet payment request
    // will start a mainnet instance and throw a "wrong network" error.
    if (arg.startsWith(BITCOIN5000_IPC_PREFIX,
                       Qt::CaseInsensitive)) // bitcoin5000: URI
    {
      savedPaymentRequests.append(arg);

      SendCoinsRecipient r;
      if (GUIUtil::parseBitcoin5000URI(arg, &r) && !r.address.isEmpty()) {
        CBitcoin5000Address address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine() {
  ...
  for (const QString &r : savedPaymentRequests) {
    ...
    if (!socket->waitForConnected(BITCOIN5000_IPC_CONNECT_TIMEOUT)) {
      delete socket;
      socket = nullptr;
      return false;
    }
    ...
    socket->write(block);
    socket->flush();
    socket->waitForBytesWritten(BITCOIN5000_IPC_CONNECT_TIMEOUT);
...
PaymentServer::PaymentServer(QObject *parent, bool startLocalServer)
  ...
  // on Mac: sent when you click bitcoin5000: links
  ...
  if (startLocalServer) {
    ...
    if (!uriServer->listen(name)) {
      ...
      QMessageBox::critical(
          ...
          tr("Cannot start bitcoin5000: click-to-pay handler"));
    } else {
...
//
// OSX-specific way of handling bitcoin5000: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager() {
  ...
  // netManager is used to fetch paymentrequests given in bitcoin5000: URIs
...
void PaymentServer::handleURIOrFile(const QString &s) {
  ...
  if (s.startsWith(BITCOIN5000_IPC_PREFIX,
                   Qt::CaseInsensitive)) // bitcoin5000: URI
  {
#if QT_VERSION < 0x050000
    QUrl uri(s);
    ...
    } else // normal URI
    {
      ...
      if (GUIUtil::parseBitcoin5000URI(s, &recipient)) {
        CBitcoin5000Address address(recipient.address.toStdString());
        if (!address.IsValid()) {
          Q_EMIT message(
              tr("URI handling"),
        ...
        Q_EMIT message(
            ...
            tr("URI cannot be parsed! This can be caused by an invalid "
               "Bitcoin5000 address or malformed URI parameters."),
            CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus &request,
  ...
  for (const std::pair<CScript, CAmount> &sendingTo : sendingTos) {
    ...
    if (ExtractDestination(sendingTo.first, dest)) {
      ...
      addresses.append(
          QString::fromStdString(CBitcoin5000Address(dest).ToString()));
    } else if (!recipient.authenticatedMerchant.isEmpty()) {
      // Unauthenticated payment requests to custom bitcoin5000 addresses are
      // not supported
    ...
    // Bitcoin5000 amounts are stored as (optional) uint64 in the protobuf
    ...
    if (IsDust(txOut, ::dustRelayFee)) {
      Q_EMIT message(
          ...
          tr("Requested payment amount of %1 is too small (considered dust).")
              .arg(Bitcoin5000Units::formatWithUnit(

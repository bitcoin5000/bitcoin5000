...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value,
  ...
  if (role == Qt::EditRole) {
    ...
    CTxDestination curAddress =
        CBitcoin5000Address(rec->address.toStdString()).Get();
    if (index.column() == Label) {
      ...
      CTxDestination newAddress =
          CBitcoin5000Address(value.toString().toStdString()).Get();
...
void AddressTableModel::updateEntry(const QString &address,
                                    ...
                                    const QString &purpose, int status) {
  // Update address book model from Bitcoin5000 core
...
QString AddressTableModel::addRow(const QString &type, const QString &label,
  ...
  if (type == Send) {
    ...
    {
      LOCK(wallet->cs_wallet);
      if (wallet->mapAddressBook.count(CBitcoin5000Address(strAddress).Get())) {
        editStatus = DUPLICATE_ADDRESS;
        return QString();
      }
    }
    ...
    strAddress = CBitcoin5000Address(newKey.GetID()).ToString();
  } else {
    return QString();
  }
  ...
  {
    LOCK(wallet->cs_wallet);
    wallet->SetAddressBook(CBitcoin5000Address(strAddress).Get(), strLabel,
...
QString AddressTableModel::labelForAddress(const QString &address) const {
  {
    LOCK(wallet->cs_wallet);
    CBitcoin5000Address address_parsed(address.toStdString());

...
#include "bitcoin5000addressvalidator.h"
...
static std::string DummyAddress(const CChainParams &params) {
  ...
  for (int i = 0; i < 256; ++i) { // Try every trailing byte
    ...
    if (!CBitcoin5000Address(s).IsValid())
      return s;
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent) {
  ...
  widget->setPlaceholderText(
      QObject::tr("Enter a Bitcoin5000 address (e.g. %1)")
          .arg(QString::fromStdString(DummyAddress(Params()))));
#endif
  widget->setValidator(new Bitcoin5000AddressEntryValidator(parent));
  widget->setCheckValidator(new Bitcoin5000AddressCheckValidator(parent));
}
...
bool parseBitcoin5000URI(const QUrl &uri, SendCoinsRecipient *out) {
  // return if URI is not valid or is no bitcoin5000: URI
  if (!uri.isValid() || uri.scheme() != QString("bitcoin5000"))
    return false;
  ...
  for (QList<QPair<QString, QString>>::iterator i = items.begin();
    ...
    if (i->first == "message") {
      ...
      if (!i->second.isEmpty()) {
        if (!Bitcoin5000Units::parse(Bitcoin5000Units::BVK, i->second,
                                     &rv.amount)) {
          return false;
        }
      }
...
bool parseBitcoin5000URI(QString uri, SendCoinsRecipient *out) {
  // Convert bitcoin5000:// to bitcoin5000:
  //
  //    Cannot handle this later, because bitcoin5000:// will cause Qt to see
  //    the part after // as host,
  //    which will lower-case it (and thus invalidate the address).
  if (uri.startsWith("bitcoin5000://", Qt::CaseInsensitive)) {
    uri.replace(0, 10, "bitcoin5000:");
  }
  QUrl uriInstance(uri);
  return parseBitcoin5000URI(uriInstance, out);
}

QString formatBitcoin5000URI(const SendCoinsRecipient &info) {
  QString ret = QString("bitcoin5000:%1").arg(info.address);
  int paramCount = 0;

  if (info.amount) {
    ret += QString("?amount=%1")
               .arg(Bitcoin5000Units::format(Bitcoin5000Units::BVK, info.amount,
                                             false,
                                             Bitcoin5000Units::separatorNever));
    paramCount++;
  }
...
bool isDust(const QString &address, const CAmount &amount) {
  CTxDestination dest = CBitcoin5000Address(address.toStdString()).Get();
...
bool openBitcoin5000Conf() {
  boost::filesystem::path pathConfig = GetConfigFile(BITCOIN5000_CONF_FILENAME);
  ...
  /* Open bitcoin5000.conf with the associated application */
...
fs::path static StartupShortcutPath() {
  ...
  if (chain == CBaseChainParams::MAIN)
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitcoin5000.lnk";
  if (chain == CBaseChainParams::TESTNET) // Remove this special case when
                                          // CBaseChainParams::TESTNET =
                                          // "testnet4"
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitcoin5000 (testnet).lnk";
  return GetSpecialFolderPath(CSIDL_STARTUP) /
         strprintf("Bitcoin5000 (%s).lnk", chain);
}

bool GetStartOnSystemStartup() {
  // check for Bitcoin5000*.lnk
...
fs::path static GetAutostartFilePath() {
  std::string chain = ChainNameFromCommandLine();
  if (chain == CBaseChainParams::MAIN)
    return GetAutostartDir() / "bitcoin5000.desktop";
  return GetAutostartDir() / strprintf("bitcoin5000-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart) {
  if (!fAutoStart)
    ...
    if (!optionFile.good())
      return false;
    std::string chain = ChainNameFromCommandLine();
    // Write a bitcoin5000.desktop file to the autostart directory:
    optionFile << "[Desktop Entry]\n";
    optionFile << "Type=Application\n";
    if (chain == CBaseChainParams::MAIN)
      optionFile << "Name=Bitcoin5000\n";
    else
      optionFile << strprintf("Name=Bitcoin5000 (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list,
                                              CFURLRef findUrl) {
  // loop through the list of startup items and try to find the bitcoin5000 app
...
bool GetStartOnSystemStartup() {
  CFURLRef bitcoin5000AppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
  ...
  LSSharedFileListItemRef foundItem =
      findStartupItemInList(loginItems, bitcoin5000AppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart) {
  ...
  if (fAutoStart && !foundItem) {
    // add bitcoin5000 app to startup item list
    LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst,
                                  nullptr, nullptr, bitcoin5000AppUrl, nullptr,
                                  nullptr);

...
static bool SignStep(const BaseSignatureCreator &creator,
                     ...
                     txnouttype &whichTypeRet, SigVersion sigversion) {
  CScript scriptRet;
  uint160 h160;
  ret.clear();
  ...
  case TX_SCRIPTHASH:
    if (creator.KeyStore().GetCScript(uint160(vSolutions[0]), scriptRet)) {
      ret.push_back(
          std::vector<unsigned char>(scriptRet.begin(), scriptRet.end()));
      return true;
    }
    return false;
...
bool ProduceSignature(const BaseSignatureCreator &creator,
  ...
  if (solved && whichType == TX_WITNESS_V0_KEYHASH) {
    ...
    txnouttype subType;
    solved = solved && SignStep(creator, witnessscript, result, subType,
  ...
  // Test solution
  return solved &&
         VerifyScript(sigdata.scriptSig, fromPubKey, &sigdata.scriptWitness,
...
static std::vector<valtype> CombineMultisig(
    ...
    const std::vector<valtype> &sigs2, SigVersion sigversion) {

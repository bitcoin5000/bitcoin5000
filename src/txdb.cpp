...
void CCoinsViewDBCursor::Next() {
  ...
  if (!pcursor->Valid() || !pcursor->GetKey(entry)) {
    keyTmp.first = 0; // Invalidate cached key after last record so that Valid()
                      // and GetKey() return false
  } else {
...
bool CBlockTreeDB::WriteBatchSync(
  ...
  CDBBatch batch(*this);
  for (std::vector<std::pair<int, const CBlockFileInfo *>>::const_iterator it =
           fileInfo.begin();
       it != fileInfo.end(); it++) {
...
public:
  ...
  //! unspent transaction outputs; spent outputs are .IsNull(); spent outputs at
  //! the end of the array are dropped

...
const char *const BITCOIN5000_CONF_FILENAME = "bitcoin5000.conf";
const char *const BITCOIN5000_PID_FILENAME = "bitcoin5000d.pid";
...
#else
  const char *pszModule = "bitcoin5000";
#endif
  if (pex)
...
// Data\Bitcoin5000
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Bitcoin5000
// Mac: ~/Library/Application Support/Bitcoin5000
// Unix: ~/.bitcoin5000
#ifdef WIN32
  // Windows
  return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitcoin5000";
#else
  fs::path pathRet;
...
#ifdef MAC_OSX
  // Mac
  return pathRet / "Library/Application Support/Bitcoin5000";
#else
  // Unix
  return pathRet / ".bitcoin5000";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string &confPath) {
  ...
  if (!streamConfig.good())
    return; // No bitcoin5000.conf file is OK
...
#ifndef WIN32
fs::path GetPidFile() {
  fs::path pathPidFile(gArgs.GetArg("-pid", BITCOIN5000_PID_FILENAME));
...
std::string CopyrightHolders(const std::string &strPrefix) {
  ...
  // Check for untranslated substitution to make sure Bitcoin5000 Core copyright
  // is not removed by accident
  if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION)
          .find("Bitcoin5000 Core") == std::string::npos) {

...
static int AppInitRawTx(int argc, char *argv[]) {
  ...
  if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||
    ...
    std::string strUsage =
        strprintf(_("%s bitcoin5000-tx utility version"), _(PACKAGE_NAME)) +
        " " + FormatFullVersion() + "\n\n" + _("Usage:") + "\n" +
        "  bitcoin5000-tx [options] <hex-tx> [commands]  " +
        _("Update hex-encoded bitcoin5000 transaction") + "\n" +
        "  bitcoin5000-tx [options] -create [commands]   " +
        _("Create hex-encoded bitcoin5000 transaction") + "\n" + "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction &tx,
  ...
  CBitcoin5000Address addr(strAddr);
  if (!addr.IsValid())
...
static void MutateTxAddOutPubKey(CMutableTransaction &tx,
  ...
  if (bScriptHash) {
    ...
    CBitcoin5000Address redeemScriptAddr(scriptPubKey);
...
static void MutateTxAddOutMultiSig(CMutableTransaction &tx,
  ...
  if (bScriptHash) {
    ...
    // Get the address for the redeem script, then call
    // GetScriptForDestination() to construct a P2SH scriptPubKey.
    CBitcoin5000Address addr(scriptPubKey);
...
static int CommandLineRawTx(int argc, char *argv[]) {
  ...
  try {
    ...
    if (!fCreateBlank) {
      ...
      // param: hex-encoded bitcoin5000 transaction

...
bool CAddrInfo::IsTerrible(int64_t nNow) const {
  if (nLastTry &&
      nLastTry >= nNow - 60) // never remove things tried in the last minute
    return false;
  ...
  if (nTime == 0 ||
      nNow - nTime >
          ADDRMAN_HORIZON_DAYS * 24 * 60 * 60) // not seen in recent history
    return true;

  if (nLastSuccess == 0 &&
      nAttempts >= ADDRMAN_RETRIES) // tried N times and never a success
    return true;
...
double CAddrInfo::GetChance(int64_t nNow) const {
  ...
  // deprioritize 66% after each failed attempt, but at most 1/28th to avoid the
  // search taking forever or overly penalizing outages.
...
void CAddrMan::MakeTried(CAddrInfo &info, int nId) {
  ...
  // first make space to add it (the existing tried entry there is moved to new,
...
void CAddrMan::Good_(const CService &addr, int64_t nTime) {
  ...
  // check whether we are talking about the exact same CService (including same
  // port)
  if (info != addr)
    return;
  ...
  if (!newOnly && (nTried > 0 && (nNew == 0 || RandomInt(2) == 0))) {
    ...
    while (1) {
      ...
      while (vvTried[nKBucket][nKBucketPos] == -1) {
        ...
        nKBucketPos =
            (nKBucketPos + insecure_rand.randbits(ADDRMAN_BUCKET_SIZE_LOG2)) %
            ADDRMAN_BUCKET_SIZE;
      }
    ...
    while (1) {
      ...
      while (vvNew[nUBucket][nUBucketPos] == -1) {
        nUBucket =
            (nUBucket + insecure_rand.randbits(ADDRMAN_NEW_BUCKET_COUNT_LOG2)) %
            ADDRMAN_NEW_BUCKET_COUNT;
        nUBucketPos =
            (nUBucketPos + insecure_rand.randbits(ADDRMAN_BUCKET_SIZE_LOG2)) %
            ADDRMAN_BUCKET_SIZE;
      }
